 ...
 * @note:  **Please do not use in interrupt. (Because the calculation time is too long and the fluctuation is too large)**
 */
static void armControlCalculate(Coordinates_t *Coordinates, float AimCenterX, float AimCenterY)
{
	if(GetCurrentTimeMillisecond() - Coordinates -> TimeStamp < (1000.0 * MaximumFPS_Fluctuation / AppleDetectionAverageFPS))
	{
		...
		//if(fabs(Coordinates -> X - xAxisPID.setpoint) > AimToleranceErrorX)
		//{
		...
		//if(fabs(Coordinates -> Y - yAxisPID.setpoint) > AimToleranceErrorX)
		//{
	...
	while((GetCurrentTimeMillisecond() - startTime) < TimeOut)
	{
		if(GetCurrentTimeMillisecond() - lastCalculateTime > 20)
		{
			...
			case Apple:
				...
				armControlCalculate(&coordinates, AppleAimCenterX, AppleAimCenterY);
				break;

			case Target:
				GetTargetCoordinates(&coordinates);
				armControlCalculate(&coordinates, TargetAimCenterX, TargetAimCenterY);
				break;
