...
def main(anchors, labels = None, model_addr="/sd/m.kmodel", sensor_window=(224, 224), lcd_rotation=0, sensor_hmirror=False, sensor_vflip=False):
    ...
    kpu.init_yolo2(task, 0.3, 0.2, 5, anchors) # threshold:[0,1], nms_value: [0, 1]
    try:
        while 1:
            img = sensor.snapshot()
            t = time.ticks_ms()
...
if __name__ == "__main__":
    try:
        labels = ['Apple', 'Banana', 'KiwiFruit', 'Lemon', 'Orange', 'Peach', 'Pear', 'Pitaya', 'SnowPear']
        #anchors = [3.8125, 3.8125, 5.375, 5.375, 7.1875, 7.1875, 11.25, 11.3125, 9.125, 9.125]
        anchors = [4.0433855056762695, 2.6101698875427246, 2.4792195558547974, 2.4517375230789185, 1.2734460830688477, 1.2734464406967165, 2.3135592937469482, 3.252824544906616, 3.0652687549591064, 3.1954803466796875]
