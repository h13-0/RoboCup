...
RoboCup::MainWorkingFlow::MainWorkingFlow(const Configs& Configs)
    ...
    , mode(WorkingMode::FruitIdentify), appleDetector(AppleDetector(Configs.GetAppleDetectorSettings()))
    , targetDetector(TargetDetectorConfigs(Configs.GetTargetDetectorSettings()))
    , FruitIdentifyFinished(0), exit(false), exited(false)
{
    using namespace cv;
    using namespace h13;
    ...
    protocol -> AddKeyCallback("CMD",
        [&](void* DataPtr, int Length)
        {
            ...
            {
                if (mode != WorkingMode::FruitIdentify)
                {
                    std::unique_lock<std::mutex> lockFlag(FruitIdentifyFinishedMutex);
                    FruitIdentifyFinished = 0;
                }
                mode = WorkingMode::FruitIdentify;
                LOG(INFO) << "Switch to mode: FruitIdentify";
            }
        }
    );
}
...
int RoboCup::MainWorkingFlow::Run()
{
    ...
    while (1)
    {
        ...
        case RoboCup::WorkingMode::WorkingMode::TargetDetection:
            targetDetect(_frame);
            break;
        case RoboCup::WorkingMode::WorkingMode::FruitIdentify:
            FruitIdentify(_frame);
            break;
        default:
            break;
        }
...
void RoboCup::MainWorkingFlow::FruitIdentify(cv::InputOutputArray InputOutputBGR_Image)
{
    using namespace std;
    using namespace cv;
    ...
    int _FruitIdentifyFinished = 1;
    {
        std::unique_lock<std::mutex> lockFlag(FruitIdentifyFinishedMutex);
        _FruitIdentifyFinished = FruitIdentifyFinished;
    }
    
    if (!_FruitIdentifyFinished)
    {
